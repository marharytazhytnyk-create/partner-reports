...
function analyzeBrandDynamics(d) {{
  ...
  // CP порівнюємо в грошах: при негативній базі відсоткова зміна вводить в оману
  // (напр. −€16 → €2 формально дає «−113%», хоча це покращення).
  ...
  if (hasOrdersDrop && aovWow != null && aovWow > 5)
    orderReasons.push(`<b>Середній чек</b> зріс на ${{fmtDeltaPct(aovWow)}} (${{fmtEurVal(aovPrev)}} → ${{fmtEurVal(aovLast)}}): можливе підняття цін у меню відлякує частину чутливих до ціни клієнтів.`);
  if (hasOrdersDrop && locDrops.length)
    orderReasons.push(`<b>Де саме падіння:</b> ${{locDrops.slice(0, 3).map(l => `${{l.name}} (${{Math.round(l.diff)}} зам., ${{fmtDeltaPct(l.pct)}})`).join('; ')}}${{locDrops.length > 3 ? ` та ще ${{locDrops.length - 3}} локац.` : ''}}.`);

  let ordersTitle = 'Чому впали замовлення';
  let ordersList  = 'Причини падіння:';
  let ordersText;
  if (oPrev == null || oLast == null) {{
    ordersTitle = 'Замовлення';
    ordersList  = '';
    ordersText  = 'Недостатньо даних для порівняння двох тижнів.';
  }} else if (hasOrdersDrop) {{
    ordersText = `Замовлення впали з ${{Math.round(oPrev)}} до ${{Math.round(oLast)}} (${{fmtDeltaPct(ordersWow)}}, ${{Math.round(ordDiff)}} шт за тиждень).`;
    if (!orderReasons.length) {{
      ordersList = '';
      orderReasons.push('Операційні метрики (availability, acceptance, failed, покази, конверсія) не погіршились — падіння радше зовнішнє: сезонність, погода, свята, активність конкурентів у зоні або завершення промо. Варто уточнити у партнера.');
    }}
  }} else {{
    ordersTitle = ordersWow != null && ordersWow > 2 ? 'Замовлення: падіння немає' : 'Замовлення: без змін';
    ordersText  = ordersWow != null && ordersWow > 2
      ? `Замовлення зросли з ${{Math.round(oPrev)}} до ${{Math.round(oLast)}} (${{fmtDeltaPct(ordersWow)}}) — пояснювати падіння немає чого.`
      : `Замовлення стабільні: ${{Math.round(oPrev)}} → ${{Math.round(oLast)}} (${{fmtDeltaPct(ordersWow)}}).`;
    ordersList = orderReasons.length ? 'Але є метрики, що тиснуть на попит:' : '';
  ...
  if (hasCpDrop && volEffect != null && rateEffect != null) {{
    ...
    if (volEffect < 0) {{
      ...
      const src = hasOrdersDrop && aovDown
        ...
        : hasOrdersDrop
          ? `менше замовлень (${{fmtDeltaPct(ordersWow)}}) — причини в блоці «Замовлення»`
          : aovDown
  ...
  const severity =
    ...
    (mLast != null && mLast < 0) || (cpPctShown != null && cpPctShown < -20)
      ? 'danger'
      : (hasOrdersDrop || hasMarginDrop || hasCpDrop) ? 'warning' : '';

  const cls = (v, badBelow, goodAbove) =>
    v == null ? 'flat' : v < badBelow ? 'down' : v > goodAbove ? 'up' : 'flat';

  const card = (icon, title, deltaHtml, deltaCls, sub, text, listTitle, reasons) => `
    <div class="dyn-analysis-card">
      ...
      ${{listTitle ? `<div class="dyn-analysis-list-title">${{listTitle}}</div>` : ''}}
  ...
  html += card('📦', ordersTitle,
    ...
    (oPrev != null && oLast != null) ? `${{Math.round(oPrev)}} → ${{Math.round(oLast)}}` : '',
    ordersText, ordersList, orderReasons);
  html += card('💶', cpTitle,
    fmtEurDelta(cpDiff), cls(cpDiff, -cpMaterial, cpMaterial),
    (cpPrev != null && cpLast != null) ? `${{fmtEurVal(cpPrev)}} → ${{fmtEurVal(cpLast)}}` : '',
    cpText, cpList, cpReasons);
  html += card('📉', mTitle,
    fmtDeltaPp(marginPp), cls(marginPp, -0.5, 0.5),
    (mPrev != null && mLast != null) ? `${{mPrev.toFixed(1)}}% → ${{mLast.toFixed(1)}}%` : '',
    mText, mList, mReasons);
  html += `</div>`;

  html += `<div class="dyn-analysis-hint">Аналіз побудовано автоматично з WoW-зміни метрик (Availability, Acceptance, Failed/Bad/Late, знижки, refunds, покази та конверсія по локаціях, середній чек). Маржа в € розкладається на ефект обсягу та ефект рентабельності; CP L2 Маржа (%) — на частку витрат у GMV. Це гіпотези для перевірки з партнером, не остаточний вердикт.</div>`;
